...
export class SubscriptionComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.api.getPlans().subscribe({
      next: p => { this.plans = p.filter((pl: SubscriptionPlan) => pl.targetRole === 'OWNER' && pl.active); this.loading = false; },
  ...
  subscribe(): void {
    ...
    this.api.subscribe(this.confirmPlan.id, this.paymentRef).subscribe({
      next: s => { this.currentSub = s; this.confirmPlan = null; this.subscribing = false; this.snack.open('Subscribed successfully!', 'OK', { duration: 3000 }); },
  ...
  daysLeft(): number {
    ...
    return Math.max(0, Math.ceil((new Date(this.currentSub.endDate).getTime() - Date.now()) / 86400000));
  }
}
